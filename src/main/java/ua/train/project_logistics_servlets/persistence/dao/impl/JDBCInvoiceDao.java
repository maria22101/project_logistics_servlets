...
public class JDBCInvoiceDao implements InvoiceDao {
    ...
    private static final String GET_INVOICE_BY_ORDER_NUMBER =
            "SELECT * FROM orders " +
                    "JOIN users " +
                    "ON orders.user_id=users.id " +
                    "JOIN addresses AS a " +
    ...
    public void payInvoice(int orderNumber)
        ...
        try (Connection connection = ConnectionPoolHolder.getConnection()) {
            ...
            try (PreparedStatement invoiceStatement = connection.prepareStatement(PAY_INVOICE);
                ...
                invoiceStatement.setInt(2, orderNumber);

                orderStatement.setString(1, OrderStatus.READY_FOR_DISPATCH.toString());
                orderStatement.setInt(2, orderNumber);
    ...
    public Optional<Invoice> getInvoiceByOrderNumber(int orderNumber)
        ...
        try (Connection connection = ConnectionPoolHolder.getConnection();
             PreparedStatement prepAddressStatement = connection.prepareStatement(GET_ADDRESS_BY_ID);
             PreparedStatement prepInvoiceStatement = connection.prepareStatement(GET_INVOICE_BY_ORDER_NUMBER)) {
            ...
            prepInvoiceStatement.setInt(1, orderNumber);
            ResultSet rs = prepInvoiceStatement.executeQuery();

            if (rs.next()) {
                invoice = invoiceMapper.extractFromResultSet(rs);
